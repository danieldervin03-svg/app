...
export default function MyCoachChatScreen() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await api.myCoachChat();
  ...
  const send = async () => {
    ...
    setInput("");
    try {
      const msg = await api.sendToMyCoach(text);
  ...
  return (
    <SafeAreaView style={styles.container} testID="my-coach-chat-screen">
      <View style={styles.header}>
        <Pressable onPress={() => router.back()} style={styles.iconBtn} testID="my-chat-back">
          <Ionicons name="chevron-back" size={22} color={colors.onSurface} />
        </Pressable>
        <Text style={styles.headerTitle} numberOfLines={1}>{user?.coach_name || "Mon coach"}</Text>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }} keyboardVerticalOffset={90}>
        {loading ? (
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <Text style={styles.emptyTxt}>Envoyez un premier message à votre coach.</Text>
            }
            renderItem={({ item }) => (
              <View style={[styles.bubbleRow, item.sender_role === "user" ? styles.bubbleRowMe : styles.bubbleRowThem]}>
                <View style={[styles.bubble, item.sender_role === "user" ? styles.bubbleMe : styles.bubbleThem]}>
                  <Text style={[styles.bubbleTxt, item.sender_role === "user" && { color: colors.onBrandPrimary }]}>
                    {item.content}
                  </Text>
                </View>
              </View>
        ...
        <View style={styles.inputRow}>
          <TextInput
            ...
            multiline
            testID="my-chat-input"
          />
          <Pressable onPress={send} disabled={sending || !input.trim()} style={[styles.sendBtn, (sending || !input.trim()) && { opacity: 0.4 }]} testID="my-chat-send">
